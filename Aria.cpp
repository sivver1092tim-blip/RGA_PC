...
__inline int EncKeySetup(const Byte *w0, Byte *e, int keyBits) {
...
__inline int DecKeySetup(const Byte *w0, Byte *d, int keyBits) {
	int  i, j, R;
	Byte t[16];
...
int EncryptBuffer(BYTE *pbSource, BYTE *pbResult, int len, BYTE* pbKey)
{
	...
	if (len % keySize != 0) return -1;

	unsigned char* roundKey = new unsigned char[keySize * (numberOfRounds + 1)];
	numberOfRounds = EncKeySetup(pbKey, roundKey, 128);
	...
	while (true)
	{
		...
		if (len >= keySize)
		{
			Crypt(pbSource, numberOfRounds, roundKey, pbResult);
			res    += keySize;
			pbSource  += keySize;
			pbResult += keySize;
			len    -= keySize;
...
int DecryptBuffer(BYTE *pbSource, BYTE *pbResult, int len, BYTE* pbKey)
{
	...
	unsigned char* roundKey = new unsigned char[keySize * (numberOfRounds + 1)];
	numberOfRounds = DecKeySetup(pbKey, roundKey, 128);
